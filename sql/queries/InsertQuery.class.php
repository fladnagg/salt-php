		...
		if ($select !== NULL) {
			...
			foreach($this->_salt_fields as $field) {
				if (!$obj->MODEL()->exists($field)) {
					throw new SaltException(L::error_query_insert_select_missing_column($field, get_class($obj)));
				}
			}
			...
			foreach($objects as $o) {
				...
				if (!$o->isNew()) {
					throw new SaltException(L::error_query_insert_not_new);
				}
				$fields = array_keys($o->getModifiedFields());
				if ((count($this->_salt_fields) != count($fields))
				|| (count(array_diff($this->_salt_fields, $fields)) > 0)) {
					throw new SaltException(L::error_query_insert_mixed_fields(implode(',', $this->_salt_fields), implode(',', $fields)));
				}
